...
module.exports = function(app, io) {
  ...
  app.route('/confirmID')
    .post(function(req, res) {
      ...
      console.log('Request to add ', requestID, ' to confirmed list...');

      if (requestID) {
        added = peerPool.confirmPeer(requestID);
      }
      ...
      if (!requestID || !added ) {
        ...
        console.log('Success - Confirmed Peers List: ', peerPool.confirmedConnectedPeers);
  ...
  app.route('/callPeer')
    .post(function(req, res) {
      ...
      console.log('Request to call from [', requestID, '] to [', calleeID, ']');

      if (requestID && calleeID) {
        success = peerPool.requestConnectPeer(requestID, calleeID);
        console.log("Success is ", success);
      }

      if (!requestID || !calleeID || !success) {
        res.send(400, { error: 'Invalid Peer IDs: ' + requestID + ', ' + calleeID} );
      } else {
        io.sockets.emit('peer_pool', peerPool.confirmedConnectedPeers);
  ...
  app.route('/callRandom')
    .post(function(req, res) {
      ...
      console.log('Request to connect ', requestID, ' to RANDOM peer...');

      if (requestID) {
        peerID = peerPool.requestRandomPeer(requestID);
      }
  ...
  app.route('/endCall')
    .post(function(req, res) {
      ...
      console.log('Request to return ', requestID, ' to connected peers');

      if (requestID) {
        confirmed = peerPool.confirmPeer(requestID);
      }
